from mochila.Cromo import Cromo
import random
import pandas as pd
...
class AG:
	...
	def selection(self, xind):
		...
		for i in range(2):
			...
			cromo.valor = self.cromos[self.ult_gera][iven[i] - 1].valor
			cromo.peso = self.cromos[self.ult_gera][iven[i] - 1].peso
			cromo.volume = self.cromos[self.ult_gera][iven[i] - 1].volume
